...
class BatchToSpace:
    ...
    def version_1(cls, ctx, node, **kwargs):
        ...
        if node.inputs[1].is_const() and node.inputs[2].is_const():
            ...
            input_block_shape = node.inputs[1]
            input_crop = node.inputs[2]
            block_shape = ctx.make_node("Cast", [input_block_shape.output[0]], {'to':TensorProto.INT64})
            crop = ctx.make_node("Cast", [input_crop.output[0]], {'to':TensorProto.INT64})
            ...
            const_one = ctx.make_const(utils.make_name("const_one"), np.array([1], dtype=np.int64))
            const_zero_three = ctx.make_const(utils.make_name("const_zero_three"), np.array([0,3], dtype=np.int64))
            block_size = ctx.make_node('Slice', [block_shape.output[0], const_zero.output[0], const_one.output[0]])
            block_prod = ctx.make_node('Mul', [block_size.output[0], block_size.output[0]])
            padded_block_prod = ctx.make_node('Pad', [block_prod.output[0], const_zero_three.output[0], const_one.output[0]], name=node.name+"_DepthToSpace_pad_1")
            new_shape_x = ctx.make_node('Div', [shape_x.output[0], padded_block_prod.output[0]])
            concat_new_shape_x = ctx.make_node('Concat', [block_shape.output[0], new_shape_x.output[0]], {'axis':0}, name=node.name+"_DepthToSpace_concat_1")
            ...
            const_one_one = ctx.make_const(utils.make_name("const_one_one"), np.array([1,1], dtype=np.int64))
            padded_block_shape = ctx.make_node('Pad', [block_shape.output[0], const_one_one.output[0], const_one.output[0]], name=node.name+"_DepthToSpace_pad_2")
            ...
            const_two = ctx.make_const(utils.make_name("const_two"),  np.array([2], dtype=np.int64))
            const_zero_zero = ctx.make_const(utils.make_name("const_zero_zero"), np.array([0,0], dtype=np.int64))
            const_one_zero = ctx.make_const(utils.make_name("const_one_zero"), np.array([1,0], dtype=np.int64))
            const_one_two = ctx.make_const(utils.make_name("const_one_two"), np.array([1,2], dtype=np.int64))
            const_two_two = ctx.make_const(utils.make_name("const_two_two"), np.array([2,2], dtype=np.int64))
            slice_crop_starts = ctx.make_node('Slice', [transposed_crop.output[0], const_zero_zero.output[0], const_one_two.output[0]])
            reshaped_slice_crop_starts = ctx.make_node('Reshape',[slice_crop_starts.output[0], const_two.output[0]])
            slice_crop_ends = ctx.make_node('Slice', [transposed_crop.output[0], const_one_zero.output[0], const_two_two.output[0]])
            reshaped_slice_crop_ends = ctx.make_node('Reshape',[slice_crop_ends.output[0], const_two.output[0]])
            const_three = ctx.make_const(utils.make_name("const_three"), np.array([3], dtype=np.int64))
            sliced_new_shape_x_v2 = ctx.make_node('Slice', [new_shape_x_v2.output[0], const_one.output[0], const_three.output[0]])
            neged_reshaped_slice_crop_ends = ctx.make_node('Sub', [sliced_new_shape_x_v2.output[0], reshaped_slice_crop_ends.output[0]])
            ctx.remove_node(node.name)
            cropped_x = ctx.make_node('Slice',[reshaped_x_v2.output[0], reshaped_slice_crop_starts.output[0],
                neged_reshaped_slice_crop_ends.output[0], const_one_two.output[0]], name=node.name, outputs=node.output)
...
class SpaceToBatch:
    ...
    def version_1(cls, ctx, node, **kwargs):
        ...
        if node.inputs[1].is_const() and node.inputs[2].is_const():
            ...
            input_x = node.inputs[0]
            input_block_shape = node.inputs[1]
            input_pad_x = node.inputs[2]
            block_shape = ctx.make_node("Cast", [input_block_shape.output[0]], {'to':TensorProto.INT64})
            pad_x = ctx.make_node("Cast", [input_pad_x.output[0]], {'to':TensorProto.INT64})
            const_zero_zero = ctx.make_const(utils.make_name("const_zero_zero"), np.array([[0,0]], dtype=np.int64))
            concated_pad_x = ctx.make_node('Concat', [const_zero_zero.output[0], pad_x.output[0]], {'axis':0}, name=node.name+"_SpaceToDepth_concat_1")
            concated_pad_x_v2 = ctx.make_node('Concat', [concated_pad_x.output[0], const_zero_zero.output[0]], {'axis':0}, name=node.name+"_SpaceToDepth_concat_2")
            transposed_concated_pad_x_v2 = ctx.make_node('Transpose', [concated_pad_x_v2.output[0]], {'perm':[1,0]})
            const_eight = ctx.make_const(utils.make_name("const_eight"), np.array([8], dtype=np.int64))
            reshaped_transposed_pad_x = ctx.make_node('Reshape', [transposed_concated_pad_x_v2.output[0], const_eight.output[0]])
            padded_input_x = ctx.make_node('Pad', [input_x.output[0], reshaped_transposed_pad_x.output[0]], name=node.name+"_SpaceToDepth_pad_1")
            const_zero = ctx.make_const(utils.make_name("const_zero"), np.array([0], dtype=np.int64))
            const_one = ctx.make_const(utils.make_name("const_one"), np.array([1], dtype=np.int64))
            const_two = ctx.make_const(utils.make_name("const_two"), np.array([2], dtype=np.int64))
            const_three = ctx.make_const(utils.make_name("const_three"), np.array([3], dtype=np.int64))
            const_four = ctx.make_const(utils.make_name("const_four"), np.array([4], dtype=np.int64))
            const_one_one = ctx.make_const(utils.make_name("const_one_one"), np.array([1,1], dtype=np.int64))
            const_one_two = ctx.make_const(utils.make_name("const_one_two"), np.array([1,2], dtype=np.int64))
            padded_block_shape = ctx.make_node('Pad',[block_shape.output[0], const_one_one.output[0], const_one.output[0]], name=node.name+"_SpaceToDepth_pad_2")
            ...
            first_row_new_shape_x = ctx.make_node('Slice', [new_shape_x.output[0], const_zero.output[0], const_two.output[0]])
            block_size = ctx.make_node('Slice', [block_shape.output[0], const_zero.output[0], const_one.output[0]])
            new_fisrt_row_new_shape_x = ctx.make_node('Concat', [first_row_new_shape_x.output[0], block_size.output[0]], {'axis':0}, name=node.name+"_SpaceToDepth_concat_3")
            second_row_new_shape_x_first_half = ctx.make_node('Slice', [new_shape_x.output[0], const_two.output[0], const_three.output[0]])
            second_row_new_shape_x_second_half = ctx.make_node('Slice', [new_shape_x.output[0], const_three.output[0], const_four.output[0]])
            new_second_row_new_shape_x_first_half = ctx.make_node('Concat', [second_row_new_shape_x_first_half.output[0], block_size.output[0]], {'axis':0}, name=node.name+"_SpaceToDepth_concat_4")
            new_second_row_new_shape_x = ctx.make_node('Concat', [new_second_row_new_shape_x_first_half.output[0], second_row_new_shape_x_second_half.output[0]], {'axis':0}, name=node.name+"_SpaceToDepth_concat_5")
            new_shape_x_v2 = ctx.make_node('Concat', [new_fisrt_row_new_shape_x.output[0], new_second_row_new_shape_x.output[0]], {'axis':0}, name=node.name+"_SpaceToDepth_concat_6")
            ...
            block_size_prod = ctx.make_node('Mul', [block_size.output[0], block_size.output[0]])
            const_zero_three = ctx.make_const(utils.make_name("const_zero_three"), np.array([0,3], dtype=np.int64))
            padded_block_size_prod = ctx.make_node('Pad', [block_size_prod.output[0], const_zero_three.output[0], const_one.output[0]], name=node.name+"_SpaceToDepth_pad_3")
            new_shape_x_v3 = ctx.make_node('Mul',[new_shape_x.output[0], padded_block_size_prod.output[0]])
            ctx.remove_node(node.name)
            new_x_v2 = ctx.make_node('Reshape', [transposed_new_x.output[0], new_shape_x_v3.output[0]], name=node.name, outputs=node.output)
