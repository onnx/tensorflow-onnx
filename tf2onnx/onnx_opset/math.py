...
class CumSum:
    ...
    def version_9(cls, ctx, node, **kwargs):
        '''
        The plan is basically to do a matmul of the input with a triangular 2-D matrix
        However prior to that we need to do a transpose to make the axis, mode to last position
        Then another transpose on the results back to the original shape
        :param ctx:
        :param node:
        :param kwargs:
        :return:
        '''
        dims = ctx.get_shape(node.input[0])
        rank = len(dims)
        dtype = utils.map_onnx_to_numpy_type(ctx.get_dtype(node.input[0]))
        axis = node.inputs[1].get_tensor_value()
        exclusive = node.get_attr_int("exclusive")
        ...
        matmul_input2 = ctx.make_const(utils.make_name("const"), tri_matrix).output[0]
    ...
    @classmethod
    def get_matrix(cls, dim, exclusive, reverse, dtype):
        ones = np.ones((dim, dim)).astype(dtype)
        if not exclusive and not reverse:
            return np.triu(ones, 0)
        if exclusive and not reverse:
            return np.triu(ones, 1)
        if not exclusive and reverse:
            return np.tril(ones, 0)
        return np.tril(ones, -1)

    @classmethod
    def get_perms(cls, rank, axis):
        baseline = list(range(0, rank))
        if axis == rank - 1:
            return baseline, baseline
        fw = baseline.copy()
        for i in range(0, rank - axis - 1):
            fw[axis + i], fw[axis + i + 1] = fw[axis + i + 1], fw[axis + i]
        bw = baseline.copy()
        for i in range(0, rank - axis - 1):
            bw[rank - 1 - i], bw[rank - 1 - i - 1] = bw[rank - 1 - i - 1], bw[rank - 1 - i]
        return fw, bw
