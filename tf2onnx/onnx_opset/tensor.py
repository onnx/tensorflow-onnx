...
class MatrixDiagPart:
    ...
    def version_11(cls, ctx, node, **kwargs):
        ...
        const_zero_zero = ctx.make_const(utils.make_name(node.name) + 'const_zero_zero', np.array([0,0]).astype(np.int64))
        ...
        const_negative_one = ctx.make_const(utils.make_name(node.name) + 'const_negative_one', np.array([-1]).astype(np.int64))
        const_negative_two = ctx.make_const(utils.make_name(node.name) + 'const_negative_two', np.array([-2]).astype(np.int64))
        const_negative_two_one = ctx.make_const(utils.make_name(node.name) + 'const_negative_two_one', np.array([-2,-1]).astype(np.int64))
        input_shape = ctx.make_node('Shape', [node.input[0]])
        input_shape_size = ctx.make_node('Shape', [input_shape.output[0]])
        matrice_shape = ctx.make_node('Slice', [input_shape.output[0], const_negative_two.output[0], input_shape_size.output[0]])
        matrice_shape_float = ctx.make_node('Cast', [matrice_shape.output[0]], attr={'to':TensorProto.FLOAT})
        matrice_shape_float_x = ctx.make_node('Slice', [matrice_shape_float.output[0], const_zero.output[0], const_one.output[0]])
        matrice_shape_float_y = ctx.make_node('Slice', [matrice_shape_float.output[0], const_one.output[0], const_two.output[0]])
        ...
        double_matrice_dim = ctx.make_node('Concat', [min_matrice_dim.output[0], min_matrice_dim.output[0]], attr={'axis':-1})
        sliced_input = ctx.make_node('Slice', [node.input[0], const_zero_zero.output[0], double_matrice_dim.output[0], const_negative_two_one.output[0]])
        sliced_input_shape = ctx.make_node('Shape', [sliced_input.output[0]])
        sliced_input_shape_half = ctx.make_node('Slice', [sliced_input_shape.output[0], const_zero.output[0], const_negative_one.output[0]])
        sliced_input_shape_new = ctx.make_node('Concat', [sliced_input_shape_half.output[0], const_one.output[0]], attr={'axis':-1})
        ...
        gathered_result = ctx.make_node('GatherElements', [sliced_input.output[0], expanded_range.output[0]], attr={'axis':-1})
        shapes = node.output_shapes
        dtypes = node.output_dtypes
        ctx.remove_node(node.name)
