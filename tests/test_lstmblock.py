...
class LSTMBlockTests(Tf2OnnxBackendTestBase):
    ...
    def test_single_dynamic_lstm_seq_length_is_const(self):
        ...
        def func(x):
            ...
            outputs, cell_state = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32, sequence_length=[4, 3, 4, 5, 2, 1])
            return tf.identity(outputs, name="output"), tf.identity(cell_state, name="cell_state")
    ...
    def test_single_dynamic_lstm_seq_length_is_not_const(self):
        ...
        def func(x, seq_length):
            # no scope
            cell = rnn.LSTMBlockCell(units)
            outputs, cell_state = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32, sequence_length=tf.identity(seq_length))
            return tf.identity(outputs, name="output"), tf.identity(cell_state, name="cell_state")
    ...
    def test_single_dynamic_lstm_time_major(self):
        ...
        def func(x):
            # no scope
            cell = rnn.LSTMBlockCell(units)
            outputs, cell_state = tf.nn.dynamic_rnn(cell, x, time_major=True, dtype=tf.float32)
            return tf.identity(outputs, name="output"), tf.identity(cell_state, name="cell_state")
    ...
    def test_single_dynamic_lstm_placeholder_input(self):
        ...
        def func(x):
            # no scope
            cell = rnn.LSTMBlockCell(units)
            outputs, cell_state = tf.nn.dynamic_rnn(cell, x, dtype=tf.float32)  # by default zero initializer is used
            return tf.identity(outputs, name="output"), tf.identity(cell_state, name="cell_state")
    ...
    def test_single_dynamic_lstm_ch_zero_state_initializer(self):
        ...
        def func(x):
            ...
            outputs, cell_state = tf.nn.dynamic_rnn(cell, x, initial_state=initial_state, dtype=tf.float32)
            return tf.identity(outputs, name="output"), tf.identity(cell_state, name="cell_state")
