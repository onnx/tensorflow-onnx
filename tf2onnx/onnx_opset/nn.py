...
class CropAndResize:
    ...
    def version_11(cls, ctx, node, **kwargs):
        ...
        mode = "nearest" if node.attr["method"] is not None and node.attr["method"].s == b"nearest" else "linear"
        extrapolation_value = float(node.attr["extrapolation_value"].f) if node.attr[
                                                                               "extrapolation_value"] is not None else 0.0
        ...
        first_box = GraphBuilder(ctx).make_slice({"data": boxes.output[0], "ends": [1, 4], "starts": [0, 0]})
        roi_raw = ctx.make_node("Reshape", [first_box, const_four.output[0]])
        roi_raw_first_half = GraphBuilder(ctx).make_slice({"data": roi_raw.output[0], "ends": [2], "starts": [0]})
        roi_raw_second_half = GraphBuilder(ctx).make_slice({"data": roi_raw.output[0], "ends": [4], "starts": [2]})
        roi_concat_1 = ctx.make_node("Concat", [const_zero_zero.output[0], roi_raw_first_half], attr={'axis': 0})
        roi_concat_2 = ctx.make_node("Concat", [const_one_one.output[0], roi_raw_second_half], attr={'axis': 0})
        ...
        final_crop_size = ctx.make_node("Concat", [first_half_of_shape, crop_size_int64.output[0]], {'axis': 0})
        resized_x = ctx.make_node("Resize", [transposed_x.output[0], final_roi.output[0], const_empty_float.output[0],
                                             final_crop_size.output[0]],
                                  attr={"mode": mode, "extrapolation_value": extrapolation_value,
                                        "coordinate_transformation_mode": "tf_crop_and_resize"})
        ctx.remove_node(node.name)
        ctx.make_node("Transpose", [resized_x.output[0]], attr={'perm': [0, 2, 3, 1]}, name=node.name,
                      outputs=node.output)
