...
class CumProd:
    ...
    def version_10(cls, ctx, node, **kwargs):
        ...
        input_rank_node = ctx.make_node('Shape', inputs=[input_shape_node.output[0]], op_name_scope=node.name,
                                        outputs=[utils.make_name('input_rank')])
        ...
        axis_length_plus_one_node = ctx.make_node('Add', inputs=[axis_length_node.output[0], one_node.output[0]],
                                                  op_name_scope=node.name, outputs=[utils.make_name('axis_length_p1')])
        ...
        loop_inputs = [num_iter_node.output[0], cond_true_node.output[0], inputs,
                       input_rank_node.output[0], axis, axis_length_plus_one_node.output[0], inputs]
        loop_outputs = [utils.make_name('loop_inputs_out'), utils.make_name('loop_rank_out'),
                        utils.make_name('loop_axis_out'), utils.make_name('loop_axis_length_plus_one_out'),
        ...
        if node.get_attr_value('exclusive'): # pad the output
            if ctx.get_dtype(loop_node.output[-1]) != ctx.get_dtype(one_node.output[0]):
                pad_const_node = ctx.make_node('Cast', inputs=[one_node.output[0]],
                                               attr={'to':  ctx.get_dtype(loop_node.output[-1])},
            ...
            output_node = ctx.make_node('Pad', op_name_scope=node.name, outputs=[utils.make_name('cumprod_out')],
                                        inputs=[loop_node.output[-1], pads_node.output[0], pad_const_node.output[0]])
    ...
    def make_loop_graph(cls, ctx, node, inputs_tensor):
        ...
        graph.add_graph_input(utils.make_name('accumulator'), ctx.get_dtype(node.input[0]),
                              ctx.get_shape(inputs_tensor))
        ...
        slice_node = graph.make_node('Slice', op_name_scope=loop_name, outputs=[utils.make_name('slice')],
                                     inputs=[graph.input_names[2], start_slice, end_slice, graph.input_names[4]])
